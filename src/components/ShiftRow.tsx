...
export default function ShiftRow({
  ...
  return (
    <motion.div
      ...
      exit={{ opacity: 0, x: -20 }}
      className="flex items-center justify-between bg-slate-700/50 rounded-xl p-3 gap-2"
    >
      {/* Shift info */}
      <div className="flex-1 min-w-0">
        ...
        <div className="text-sm text-slate-300">
          ...
          {shift.role && (
            <span className="text-slate-400 mr-2">| {shift.role}</span>
          )}
        </div>
        {shift.note && (
          <p className="text-xs text-slate-400 mt-1">{shift.note}</p>
        )}
      </div>
